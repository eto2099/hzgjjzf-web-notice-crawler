...
class HZGJJSpider(BaseSpider):
    ...
    def crawl(self, target_date: Optional[str] = None):
        logger.info("start crawling Hangzhou provident fund law enforcement notices")
        if target_date:
            try:
        ...
        if target_date:
            ...
            if latest_date < today:
                logger.info(
                    ...
                    f"fall back to latest announcement date: {latest_date}"
                )
    ...
    def _fetch_page(self, page: int) -> list[dict]:
        ...
        data = self.request(settings.API_URL, params=params)
